...
rules.forEach(rule => {
  if(rule.split("contain")[0].includes("shiny gold")) {
    let bagName = rule.split(" contain ")[0].split(" bags ")[0];
    let containerBags = rule.split(" contain ")[1].split(", ").map(bag => bag.replace(".", ""));
    let containerBagObjs = [];
    containerBags.forEach(bag => {
      let num = Number(bag.split(" ")[0]) == NaN ? 0 : Number(bag.split(" ")[0]);
      let bagVal = Number(bag.split(" ")[0]) == NaN ? "None" : bag.split(" ")[1] + " " + bag.split(" ")[2] + " bags";
      containerBagObjs.push({num: num, val: bagVal});
    })
    // console.log(bagName, containerBagObjs)
    let node = new Node({num: 1, val: bagName, sum: 1});
    // containerBagObjs.forEach(obj => {
    //   node.insert(obj.val)
    // })
    node = findSum(bagName, containerBagObjs, node, 1);
    // console.log(node.getValue())
    // console.log(node)
    // traverseTree(node)
    printTree(
      node,
      node => node.value.val + " - " + node.value.num,
      node => node.children
    )
...
function findSum(bagName, containerBagObjs, parentNode, sum) {
  // console.log(bagName, containerBagObjs, parentNode);
  containerBagObjs.forEach(bagObj => {
    let currentTotal = bagObj.num
    let add = sum * currentTotal;

    ans += add;
    let node = new Node({num: bagObj.num, val: bagObj.val, sum: sum});
    // parentNode.insert(node);
    // console.log(bagObj)
    if(bagObj.num == 0) return parentNode;
    rules.forEach(rule => {
      if(rule.split("contain")[0].includes(bagObj.val)) {
        let bagName = rule.split(" contain ")[0].split(" bags ")[0];
        let containerBags = rule.split(" contain ")[1].split(", ").map(bag => bag.replace(".", ""));
        let containerBagObjs = [];
        containerBags.forEach(bag => {
          let num = isNaN(Number(bag.split(" ")[0])) ? 0 : Number(bag.split(" ")[0]);
          let bagVal = isNaN(Number(bag.split(" ")[0])) ? "None" : bag.split(" ")[1] + " " + bag.split(" ")[2] + " bags";
          containerBagObjs.push({num: num, val: bagVal});
        })
        // console.log(bagName, containerBagObjs)
        parentNode.insert(findSum(bagName, containerBagObjs, node, add));
      }
    })
  })
  // console.log(sum)
