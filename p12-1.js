...
function changeDir(amount, currIndex) {
  if(amount > 0) {
...
for(let i = 0; i < dirList.length; i++) {
  ...
  if(instr == "F") {
    instr = Object.keys(directions)[current];
  }

  if(instr == "R") {
    current = changeDir(amount / 90, current);
  } else if(instr == "L") {
    current = changeDir(-(amount / 90), current);
  } else if(instr == "W") {
    directions["W"] += ((amount - directions["E"]) < 0 ? 0 : (amount - directions["E"]));
    directions["E"] = (directions["E"] > amount ? directions["E"] - amount : 0);
  } else if(instr == "N") {
    directions["N"] += ((amount - directions["S"]) < 0 ? 0 : (amount - directions["S"]));
    directions["S"] = (directions["S"] > amount ? directions["S"] - amount : 0);
  } else if(instr == "E") {
    directions["E"] += ((amount - directions["W"]) < 0 ? 0 : (amount - directions["W"]));
    directions["W"] = (directions["W"] > amount ? directions["W"] - amount : 0);
  } else if(instr == "S") {
    directions["S"] += ((amount - directions["N"]) < 0 ? 0 : (amount - directions["N"]));
    directions["N"] = (directions["N"] > amount ? directions["N"] - amount : 0);
  }
