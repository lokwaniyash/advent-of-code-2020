...
rules.forEach(rule => {
  if(rule.split("contain")[1].includes("shiny gold")) sBagOuter.push(rule.split("contain")[0].split(" bags ")[0]);
})

sum = sBagOuter.length;
holdBags = sBagOuter
console.log(sBagOuter)
...
while(true) {
  rules.forEach(rule => {
    flag = false;
    sBagOuter.forEach(bag => {
      if(flag) return;
      if(rule.split("contain")[1].includes(bag)) {
        shinyBags.push(rule.split("contain")[0].split(" bags ")[0]);
        holdBags.push(rule.split("contain")[0].split(" bags ")[0]);
        flag = true;
      }
    })
    // sum++
  ...
  shinyBags = [];
  sum += sBagOuter.length
  // holdBags.push(sBagOuter)
  // console.log(sBagOuter.length, sBagOuter)
  if(sBagOuter.length == 0) break;
}

let finalBags = [];
holdBags.forEach(bag => {
  if(!finalBags.includes(bag)) finalBags.push(bag)
})

// console.log(sBagOuter, sum)
console.log(finalBags.length);

// console.log(shinyBags, shinyBags.length)