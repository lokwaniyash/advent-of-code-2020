...
values.forEach(passStr => {
  // console.log(pass)
  pass += passStr.endsWith(" ") || pass == "" || passStr == "" ? passStr : " " + passStr;
  if(passStr == "") {
    passArr = pass.split(" ")
    // console.log(passArr)
    // passNo++
    ...
    passArr.forEach(passVal => {
      let field = passVal.split(":")[0]
      let val = passVal.split(":")[1]

      if(field == "byr") {
        // console.log(val)
        Number(val) >= 1920 && Number(val) <= 2002 ? flag++ : "";
      }

      if(field == "iyr") {
        // console.log(val)
        Number(val) >= 2010 && Number(val) <= 2020 ? flag++ : "";
      }

      if(field == "eyr") {
        Number(val) >= 2020 && Number(val) <= 2030 ? flag++ : "";
      }

      if(field == "hgt") {
        // console.log(val)
        let meas = val.substring(Math.max(val.length - 2, 0));
        if(meas == "cm") {
          let num = val.substring(0, 3);
          Number(num) >= 150 && Number(num) <= 193 ? flag++ : "";
        } else if(meas == "in") {
          let num = val.substring(0, 2);
          Number(num) >= 59 && Number(num) <= 76 ? flag++ : "";
        }
      }

      if(field == "hcl") {
        /^#[0-9A-F]{6}$/i.test(val) ? flag++ : "";
      }

      if(field == "ecl") {
        val == "amb" || val == "blu" || val == "brn" || val == "gry" || val == "grn" || val == "hzl" || val == "oth" ? flag++ : "";
      }
