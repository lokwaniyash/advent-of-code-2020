...
let sum = 0;
answers.forEach(ans => {
  if(ans != "") eachAns.push(ans);

  if(ans == "") {
    // console.log(eachAns);
    // eachAns = ["abc", "bcd", "cdf"]
    eachAns.forEach(each => {
      Array.from(each).forEach(ch => {
        let flag = true;
        eachAns.forEach(checkAns => {
          if(!checkAns.includes(ch)) flag = false;
        })
        if(flag && !str.includes(ch)) str += ch;
      })
    })
